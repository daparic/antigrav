...
void generate_moves(const Board &board, Moves &moves) {
  ...
  for (int piece = P; piece <= k; piece++) {
    ...
    if (board.side == 0) { // White
      if (piece >= p)
        continue; // Skip black pieces
    } else {      // Black
      if (piece < p)
        continue; // Skip white pieces
    }

    while (bitboard) {
      // Get source square (LSB)
      source_square = count_trailing_zeros(bitboard);
      ...
      if (piece == P) { // White Pawn
        ...
        if (target >= 0 && !get_bit(board.occupancies[2], target)) {
          ...
          if (source_square >= a7 && source_square <= h7) {
            add_move(moves,
                     encode_move(source_square, target, piece, Q, 0, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, R, 0, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, B, 0, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, N, 0, 0, 0, 0));
          } else {
            add_move(moves,
                     encode_move(source_square, target, piece, 0, 0, 0, 0, 0));
            // 2. Double Push (from Rank 2: 48-55)
            if ((source_square >= a2 && source_square <= h2) &&
                !get_bit(board.occupancies[2], target - 8)) {
              add_move(moves, encode_move(source_square, target - 8, piece, 0,
                                          0, 1, 0, 0));
            }
          }
        }
        ...
        // 3. Captures
        attacks = pawn_attacks[0][source_square] &
                  board.occupancies[1]; // White pawns capture black
        while (attacks) {
          target = count_trailing_zeros(attacks);
          if (source_square >= a7 && source_square <= h7) {
            add_move(moves,
                     encode_move(source_square, target, piece, Q, 1, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, R, 1, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, B, 1, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, N, 1, 0, 0, 0));
          } else {
            add_move(moves,
        ...
        if (target < 64 && !get_bit(board.occupancies[2], target)) {
          ...
          if (source_square >= a2 &&
              source_square <= h2) { // 2nd rank from top (7th rank)
            add_move(moves,
                     encode_move(source_square, target, piece, q, 0, 0, 0, 0));
            add_move(moves,
                     encode_move(source_square, target, piece, r, 0, 0, 0, 0));
        ...
        // 3. Captures
        attacks = pawn_attacks[1][source_square] &
                  board.occupancies[0]; // Black pawns capture white
        while (attacks) {
          target = count_trailing_zeros(attacks);
          if (source_square >= a2 && source_square <= h2) {
            add_move(moves,
        ...
        if (piece == N || piece == n)
          ...
          attacks = get_bishop_attacks(source_square, board.occupancies[2]);
        else if (piece == R || piece == r)
          attacks = get_rook_attacks(source_square, board.occupancies[2]);
        else if (piece == Q || piece == q)
          attacks = get_queen_attacks(source_square, board.occupancies[2]);
        ...
        while (attacks) {
          ...
          // Capture logic
          int capture = 0;
          if (get_bit(board.occupancies[1 - board.side], target))
            capture = 1;

          add_move(moves, encode_move(source_square, target, piece, 0, capture,
                                      0, 0, 0));
  ...
  if (board.side == 0) { // White Castling
    // King side (e1 -> g1)
    if ((board.castle & 1) && !get_bit(board.occupancies[2], f1) &&
        !get_bit(board.occupancies[2], g1)) {
      // Check attacks (e1, f1, g1 must not be attacked)
      if (!is_square_attacked(e1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(f1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(g1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies)) {
        add_move(moves, encode_move(e1, g1, K, 0, 0, 0, 0, 1));
      }
    }
    // Queen side (e1 -> c1)
    if ((board.castle & 2) && !get_bit(board.occupancies[2], d1) &&
        !get_bit(board.occupancies[2], c1) &&
        !get_bit(board.occupancies[2], b1)) {
      if (!is_square_attacked(e1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(d1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          // c1 check not required by FIDE rules? "squares that the king passes
          // over or lands on". King moves e1->d1->c1. So d1 and c1 must be
          // safe. b1 is irrelevant to King path (rook moves over it)
          !is_square_attacked(c1, 1, (U64 *)board.bitboards,
                              (U64 *)board.occupancies)) {
    ...
    // King side (e8 -> g8)
    if ((board.castle & 4) && !get_bit(board.occupancies[2], f8) &&
        !get_bit(board.occupancies[2], g8)) {
      if (!is_square_attacked(e8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(f8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(g8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies)) {
        add_move(moves, encode_move(e8, g8, k, 0, 0, 0, 0, 1));
      }
    }
    // Queen side (e8 -> c8)
    if ((board.castle & 8) && !get_bit(board.occupancies[2], d8) &&
        !get_bit(board.occupancies[2], c8) &&
        !get_bit(board.occupancies[2], b8)) {
      if (!is_square_attacked(e8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(d8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies) &&
          !is_square_attacked(c8, 0, (U64 *)board.bitboards,
                              (U64 *)board.occupancies)) {
...
int make_move(Board &board, int move, int capture_flag) {
  ...
  if (source < 0 || source > 63) {
    std::cout << "Invalid source " << source << "\n";
    return 0;
  }
  ...
  if (promoted) {
    if (promoted < 0 || promoted > 11) {
      std::cout << "Invalid promoted " << promoted << "\n";
      return 0;
    }
  ...
  for (int i = P; i <= K; i++)
    board.occupancies[0] |= board.bitboards[i];
  for (int i = p; i <= k; i++)
    board.occupancies[1] |= board.bitboards[i];
  ...
  int king_sq = -1;
  if (board.side ==
      1) { // Was White's turn, now Black. Check if White king is attacked
    if (board.bitboards[K])
      king_sq = count_trailing_zeros(board.bitboards[K]);
    if (king_sq != -1 &&
        is_square_attacked(king_sq, 1, board.bitboards, board.occupancies))
      return 0; // Illegal
  } else {      // Was Black's turn, now White. Check if Black king is attacked
    if (board.bitboards[k])
      king_sq = count_trailing_zeros(board.bitboards[k]);
    if (king_sq != -1 &&
        is_square_attacked(king_sq, 0, board.bitboards, board.occupancies))
      return 0; // Illegal
  }
