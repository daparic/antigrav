...
int check_game_state(Board &board) {
  ...
  if (is_square_attacked(king_sq, board.side ^ 1, board.bitboards,
                         board.occupancies)) {
    return 1; // Checkmate
  } else {
    return 0; // Stalemate
...
void solve_mate_in_2(char *fen) {
  ...
  for (int i = 0; i < root_moves.count; i++) {
    ...
    if (!make_move(b1, m1, get_move_capture(m1)))
    ...
    if (check_game_state(b1) == 0)
      continue; // Stalemate, not mate

    // 3. This is now Black's turn.
    // We need to verify that FOR ALL Black legal moves, White has a responding
    // Mate in 1.
    ...
    // Verify at least one black move exists (already checked by
    // check_game_state != -1) But let's be safe.
    ...
    for (int j = 0; j < black_moves.count; j++) {
      ...
      if (!make_move(b2, m2, get_move_capture(m2)))
        continue;
